...
function ENT:SpawnFunction( ply, tr, ClassName )
	...
	ent:Spawn()
	ent:Activate()
	ent:SetColor(Color(129, 140, 82, 255))

	return ent
end

function ENT:OnTick()
	local Pod = self:GetGunnerSeat( Pod )
	local Gunner = self:GetGunner()
	
	if not IsValid( Pod ) or not IsValid( Gunner ) then return end
	...
	self:SetPoseParameter("turrets_yaw_l", self.sm_pp_yaw )
	self:SetPoseParameter("turrets_pitch_l", self.sm_pp_pitch )
...
if self:GetAI() then return end
	...
	local ent = ents.Create( "drc_snow_sparrowhawk_laser" )
	local Pos = self:LocalToWorld( Vector(210,0,-80) )
	ent:SetPos( Pos )
	...
	if tr.Hit then
		...
		if IsValid( Target ) then
			if Target:GetClass():lower() ~= "lfs_missile_gaussrocket_forked" then
				ent:SetLockOn( Target )
				ent:SetStartVelocity( 90000000 )
			end
		end
	end
