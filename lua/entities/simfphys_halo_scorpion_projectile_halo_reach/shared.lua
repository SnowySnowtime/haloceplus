...
function ENT:Initialize()
    if SERVER then
	 util.SpriteTrail( self, 0, Color(255,255,255,155), false, 10, 0, .9, 1, "trails/smoke" )
	...
	sound.Play( "halo/halo_3/missile_launch" .. math.random(1,4) .. ".ogg",  self:GetPos(), 100, 100 )
        self.kt = CurTime() + self.FuseTime
        self.motorsound = CreateSound( self, "halo/halo_3/missile_loop.ogg")
    end
...
function ENT:Detonate()
    if SERVER then
        ...
        if self:WaterLevel() >= 1 then
            ...
            util.Effect( "effect_simfphys_halo_scorpion_projectile_explosion_hreach", effectdata)
	sound.Play( "halo/halo_3/rocket_expl_h3_" .. math.random(1,3) .. ".ogg",  self:GetPos(), 100, 100 )
        ...
        util.BlastDamage(self, attacker, self:GetPos(), 225, 1000)
	 util.ScreenShake(self:GetPos(),10000,100,0.6,1048)
        self:Remove()
    end
end
    ...
    if CLIENT then
        ...
        if self.Armed then
            cam.Start3D() -- Start the 3D function so we can draw onto the screen.
                ...
                render.DrawSprite( self:GetPos(), math.random(75, 100), math.random(105, 110), Color(255, 225, 150) ) -- Draw the sprite in the middle of the map, at 16x16 in it's original colour with full alpha.
            cam.End3D()
        end
    end
end