...
SWEP.PrintName			= "BR85-SPR"
SWEP.InfoName			= "Battle Rifle - Range"
...
SWEP.UseHands			= true
SWEP.ViewModel = 		"models/snowysnowtime/drc/ho/c_br_rng.mdl"
SWEP.WorldModel			= "models/snowysnowtime/drc/ho/w_br_rng.mdl"
...
SWEP.Primary.Sound 			= Sound("drc.HaloOnline_BR_range")
...
SWEP.VElements = {
	["ammo_counter"] = { type = "Quad", bone = "b_gun", rel = "", pos = Vector(1.4, -.23, 6.13), angle = Angle(0, -90, 45), size = 0.01, draw_func = nil},
	...
	if CLIENT then
		self.VElements["ammo_counter"].draw_func = function( weapon )
			if self:Clip1() < 10 then
				draw.SimpleText("0".. self:Clip1() .."", "343_ammocounter", 0, 8.5, Color(100,0,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
			else
				draw.SimpleText(self:Clip1(), "343_ammocounter", 0, 8.5, Color(100,0,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
			end
		end
		
		self.VElements["ammo_counter_00"].draw_func = function( weapon )
			draw.SimpleTextOutlined("00", "343_ammocounter", 0, 8.5, Color(100,0,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, Color(115, 186, 214,150))
		end
	end
end