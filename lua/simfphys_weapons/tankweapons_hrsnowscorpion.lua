...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	local projectile = {}
		...
		projectile.BlastEffect = "simfphys_h3_snow_expl"
...
function simfphys.weapon:ValidClasses()
	return { "simfphys_hrscorpionsnow" }
end

function simfphys.weapon:Initialize( vehicle )
	net.Start( "halo_misc_register_tank" )
		net.WriteEntity( vehicle )
		net.WriteString( "hrscorpionsnow" )
	net.Broadcast()
	local data = {}
	...
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(-20,0,6), Vector(0,-5,15), true, "mgmuzzle" )
end
...
function simfphys.weapon:ControlMachinegun( vehicle, deltapos )
	...
	local Rate = FrameTime() / 5
	vehicle.smTmpMG = vehicle.smTmpMG and vehicle.smTmpMG + math.Clamp((fire and 1 or 0) - vehicle.smTmpMG,-Rate * 0.5,Rate) or 0
...
function simfphys.weapon:Attack( vehicle, ply, shootOrigin, Attachment, ID )
	...
	self:SetNextFire( vehicle, CurTime() + 0.20 + (vehicle.smTmpMG) * -0.125 )
	
	ParticleEffectAttach( "simphys_halo_scorpion_chaingun_muzzle", PATTACH_POINT_FOLLOW, vehicle, 3 )
end
...
function simfphys.weapon:PrimaryAttack( vehicle, ply, shootOrigin, Attachment )
	...
	ParticleEffectAttach( "simphys_halo_scorpion_muzzle", PATTACH_POINT_FOLLOW, vehicle, 4 )
	
	vehicle:PlayAnimation( "sh00t" )
	cannon_fire( ply, vehicle, shootOrigin + shootDirection * 80, shootDirection )
	
	self:SetNextPrimaryFire( vehicle, CurTime() + 3.25 )
end
...
function simfphys.weapon:ControlTrackSounds( vehicle, wheelslocked ) 
	...
	local fastenuf = speed > 10 and not wheelslocked and self:IsOnGround( vehicle )
