...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	SWTYR.FirePhysProjectile( projectile )
end
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(0,0,0), Vector(0,0,0), true, "muzzle" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(0,0,20), Vector(50,70,140), true, "muzzle" )
