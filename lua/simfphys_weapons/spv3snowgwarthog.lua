...
function simfphys.weapon:PrimaryAttack( vehicle, ply )
	...
	local AttachmentID = vehicle.swapMuzzle and vehicle:LookupAttachment( "muzzle1" ) or vehicle:LookupAttachment( "muzzle1" )
	local Attachment = vehicle:GetAttachment( AttachmentID )
	
	local shootOrigin = Attachment.Pos + deltapos * engine.TickInterval()
	local shootDirection = Attachment.Ang:Forward()
	
	ParticleEffectAttach( "simphys_halo_warthog_gauss_muzzle", PATTACH_POINT_FOLLOW, vehicle, 7 )
